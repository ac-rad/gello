...
def load_trajectory(pkl_files):
    ...
    for pkl in pkl_files:
        ...
        action = demo.get('control')
        if action is not None:
            actions_list.append(action)
    return np.array(actions_list)

def process_trajectories(data_dir, output_dir):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    traj_dirs = natsorted(glob.glob(os.path.join(data_dir, '*/')))
    for idx, traj_dir in enumerate(traj_dirs, start=1):
        pkl_files = natsorted(glob.glob(os.path.join(traj_dir, '*.pkl')))
        actions = load_trajectory(pkl_files)

        if actions.size == 0:
            continue

        np.save(os.path.join(output_dir, f'traj_{idx}_actions.npy'), actions)
        ...
        for j in range(actions.shape[1]):
            plt.plot(actions[:, j], label=f'Joint {j+1}')

        plt.title(f'Trajectory {idx} Actions')
        plt.xlabel('Timestep')
        plt.ylabel('Action values')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(output_dir, f'traj_{idx}_actions.png'))
        plt.close()

    print(f"All trajectories processed and saved in '{output_dir}' ")


if __name__ == "__main__":
    data_directory = "bc_data/gello"
    output_directory = "/gello/processed_data"

    process_trajectories(data_directory, output_directory)
